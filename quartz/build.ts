...
async function buildQuartz(argv: Argv, mut: Mutex, clientRefresh: () => void) {
	...
	if (argv.fastRebuild) {
		...
		for (const emitter of cfg.plugins.emitters) {
			dependencies[emitter.name] = (await emitter.getDependencyGraph?.(ctx, filteredContent, staticResources)) ?? null;
		}
	}
...
async function partialRebuildFromEntrypoint(
	...
	switch (action) {
		case 'add':
			...
			for (const emitter of cfg.plugins.emitters) {
				const emitterGraph = (await emitter.getDependencyGraph?.(ctx, processedFiles, staticResources)) ?? null;
	...
	for (const emitter of cfg.plugins.emitters) {
		...
		if (depGraph === null) {
			if (argv.verbose) {
				console.log(`Emitter ${emitter.name} doesn't define a dependency graph. Calling it with all files...`);
			}

			const files = [...contentMap.values()].filter(([_node, vfile]) => !toRemove.has(vfile.data.filePath!));
...
async function rebuildFromEntrypoint(
	...
	const { ctx, ignored, mut, initialSlugs, contentMap, toRebuild, toRemove, trackedAssets } = buildData;
