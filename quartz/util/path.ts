...
function sluggify(s: string): string {
	return s
		.split('/')
		.map((segment) =>
			segment.replace(/\s/g, '-').replace(/&/g, '-and-').replace(/%/g, '-percent').replace(/\?/g, '').replace(/#/g, '')
		)
...
const _rebaseHastElement = (el: HastElement, attr: string, curBase: FullSlug, newBase: FullSlug) => {
...
export function normalizeHastElement(rawEl: HastElement, curBase: FullSlug, newBase: FullSlug) {
	...
	if (el.children) {
		el.children = el.children.map((child) => normalizeHastElement(child as HastElement, curBase, newBase));
	}
